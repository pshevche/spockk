...
Failure-message rendering is notably *not* reimplemented in Kotlin: the generated bytecode calls Spock's own
`ValueRecorder`/`SpockRuntime`/`Condition` runtime classes directly, so assertion diagrams match real Spock's
output exactly. See [`spockk-compiler-plugin`](spockk-compiler-plugin/README.adoc) for how this constrains the IR
transformation.
