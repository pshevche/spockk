...
This file is the single source of truth for project instructions. Claude Code is the only coding agent used on this
project. Skills live under `.claude/skills/`, shared instructions under `.claude/instructions/`, and plugin
configuration in `.claude/settings.json`.

@.claude/instructions/conventional-commits.md
...
- **`gh-issue`** - hand off a GitHub issue to Claude Code for implementation
...
Use the `/gh-issue` skill to hand off a GitHub issue:
...
The `/gh-issue` skill runs the Superpowers `brainstorming` skill for `type::story` issues, then `writing-plans`
to turn the approved design into an implementation plan.
