...
class SpockkTestEngineInheritanceTest : Specification() {
  fun `discovers feature methods defined both in parent and child`() {
    ...
    then
    events.assertStatistics { it.started(4).succeeded(2).failed(2) }
  }
  ...
  fun `discovers feature methods by name from parent class`() {
    `when`
    val events =
      execute(selectMethod(InheritedOpenChildSpec::class.java, "successful parent feature"))
  ...
  fun `executes inherited features before own features`() {
    `when`
    val features =
      execute(selectClass(InheritedOpenChildSpec::class.java))
        .started()
