...
- Every upstream feature is either ported, explicitly marked not-applicable with a reason, or linked to a tracked
  Spockk gap. No silent omissions.
...
`_docs/migration/spock-inventory.json`, checked in and regenerated by the weekly job:
...
A Python script under `.github/migration/`. Python rather than Kotlin because it runs in CI and in the reconciler
...
Scope lives in `.github/migration/config.yml`, not in code, so changing it is a reviewable diff.
...
| `migration::blocked` | every remaining feature is blocked by an open gap |

`migration::blocked` is computed, not set by hand: when the last unported feature in an issue is linked to an open
gap, the reconciler applies it, and removes it when a gap closes. The delegation queue is
`migration::spec` minus `migration::blocked`.
...
### Ported
A Kotlin test exists carrying `@MigratedFrom`. Checkbox ticks automatically.
...
Recorded in `_docs/migration/exclusions.yml`:
...
For the `ast-snapshot` recipe the annotation has no Kotlin declaration to attach to, since the artifact is a pair
of resource files. There the key goes in a header comment in the sample source file, and the scanner reads it from
there.
...
The router, invoked when working a `migration::spec` ticket. `SKILL.md` covers classification, file placement and
naming, the `@MigratedFrom` and `@PendingFeature` conventions, the verification loop (including the `--rerun`
caveat for `compileTestFixturesKotlin` already documented in `CLAUDE.md`), and the definition of done. Detail is
pushed into references so the top-level file stays readable:
...
- `references/groovy-to-kotlin.md` is the substantial one, and the part most likely to determine whether ports are
  ...
  lambdas, operator overloading differences, property access versus getters, named and default arguments, spread,
  and where Spockk's power-assert rendering diverges from Groovy's.
...
One issue produces one PR touching one test file (plus fixtures). This is what makes roughly 400 tickets (365
classes, plus part-issues from section 7.3) tractable in parallel: the only shared files are the exclusions ledger
and occasional new fixtures, so conflicts stay rare.
...
| ~400 issues overwhelm the issue tracker | Three-tier hierarchy, `migration::blocked` keeps unworkable tickets out of the queue, and the areas roll up |
...
These need a call before implementation. My recommendation is first in each case.

1. **Bootstrap scope.** Create all 365 class issues at once, or gate on areas where Spockk support exists today and
   hold the rest until their gaps close? *Recommendation: create all of them, but let the computed
   `migration::blocked` label keep the unworkable ones out of the delegation queue. It keeps the dashboard honest
   about total scope from day one, and the label already does the filtering.*

2. **`util-api` (10 classes, 87 features).** `spock.util.concurrent` (`BlockingVariable`, `PollingConditions`,
...
3. **Splitting threshold.** 20 features per issue, giving roughly 40 extra part-issues. *Recommendation: start at
   20, adjust after the first area completes and there is real data on how long a ticket takes.*

4. **Manifest location.** `_docs/migration/` alongside the specs, or `.github/migration/` next to the tooling?
   *Recommendation: tooling and config in `.github/migration/`, generated data and the exclusions ledger in
   `_docs/migration/`, since the ledger is reviewed as documentation rather than code.*
...
2. **Traceability.** `@MigratedFrom`, the scanner, CI validation, and the exclusions ledger format. Retrofit
   annotations onto the handful of `spockk-specs` tests already derived from upstream, which validates the whole
   mechanism against real cases before anything depends on it.
