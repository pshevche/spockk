...
. link:01-plan-block-labels.adoc[Add setup and cleanup block labels] — core library + compiler plugin registration + block validation state machine
. link:02-plan-fixture-method-detection.adoc[Add fixture method detection] — collection phase: detect, validate, and track fixture methods
. link:03-plan-fixture-method-transformation.adoc[Implement fixture method IR transformation] — de-virtualization and field access validation
. link:04-plan-cleanup-block-transformation.adoc[Implement cleanup block IR transformation] — try-catch-finally IR generation
. link:05-plan-smoke-and-runtime-tests.adoc[Add smoke and runtime tests] — end-to-end and engine-level test coverage
. link:06-plan-intellij-plugin.adoc[Update IntelliJ plugin] — suppress warnings for new block label FQNs
