...
That's a complete Kotlin class — `Specification` is Spock's own base class, `given`/`when`/`then`/`and` are ordinary Kotlin declarations, and the string in parentheses is a natural-language description of the block. No annotations, no reflection tricks, no Groovy in sight.
